...
#
# dep = [PARAMETERS['depth_s'],45000]
# epi = [PARAMETERS['epi'], 37]
#
total_CC = np.array([])
total_L2 = np.array([])
total = np.array([])
...
with open(file_path, 'w') as save_file:
    for i in range(100):
        for j in range(100):
            ...
            traces_syn, p_syn, s_syn = window_code.get_window_obspy(tr_syn,epi[j],dep[i],time_at_receiver,VALUES['npts'])
