...
class Misfit:
    ...
    def R_CC(self, R_env_obs, R_env_syn):
        # I suppose that your observed traces and synthetic traces are filtered with same bandwidths in same order!

        R_dict = {}
        misfit = np.array([])
        for i in range((len(R_env_obs))):
            dt = R_env_obs[i].meta.delta

            cc_obspy = cc.correlate(R_env_obs[i].data, R_env_syn[i].data, int(0.25 * len(R_env_obs[i].data)))
            shift, CC = cc.xcorr_max(cc_obspy)

            s_syn_shift_obspy = self.shift(R_env_syn[i].data, -shift)

            D = 1 - CC  # Decorrelation
            misfit = np.append(misfit, ((CC - 0.95) ** 2) / (2 * (0.1) ** 2) + np.abs(shift))
            R_dict.update({'%s' % R_env_obs.traces[i].stats.channel: {'misfit': misfit[i], 'time_shift': shift}})
        sum_misfit = np.sum(misfit)
        return sum_misfit

    def R_L2(self, R_env_obs, R_env_syn, var):
        misfit = np.array([])
        for i in range(len(R_env_obs)):
            dt = R_env_obs[i].meta.delta
            ...
            R_syn_shift = self.shift(R_env_syn[i].data, -shift)

            d_obs_mean = np.mean(R_env_obs[i].data)
            var_array = var * d_obs_mean

            misfit = np.append(misfit,
                               np.matmul((R_env_obs[i].data - R_syn_shift).T, (R_env_obs[i].data - R_syn_shift)) / (
                                   2 * (var_array ** 2)))
            time = -shift * dt
            # plt.plot(R_syn_shift,label='R_syn_shifted')
            # plt.plot(R_env_syn[i],label = 'R_syn')
            # plt.plot(R_env_obs[i],label = 'R_obs')
            # plt.legend()
            # plt.show()
            # plt.close()
        sum_misfit = np.sum(misfit)
